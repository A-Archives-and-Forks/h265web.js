...
如果你的流里带有额外 SEI 元数据，可以继续接这种功能回调。
...
| `hls_strategy` | `string` | 否 | HLS 路由策略：`auto`（默认）、`mainline` 或 `legacy` |
...
| `hls_strategy: 'auto'` | 根据浏览器能力、指定的 `core` 和已绑定回调选择兼容的主线路由；不会加载 `past-core/ext*` |
